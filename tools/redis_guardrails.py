...
def check_redis_notes(findings: list[Finding]) -> None:
    readme = REPO_ROOT / "README.md"
    if not readme.exists():
        return
    text = read_text(readme)
    if "Redis" not in text and "redis" not in text:
        add(findings, "WARN", "docs.redis", "README should mention Redis and how production validations are performed safely.", readme)
    if re.search(r"(?i)postgres|pgbouncer|pg_dump|psql", text):
        add(findings, "ERROR", "docs.mismatch", "README references Postgres tooling; this repo is Redis-focused.", readme)


def main() -> int:
    parser = argparse.ArgumentParser(description="Offline Redis governance guardrails for this repo.")
